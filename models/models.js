...
//Los usuarios pueden tener varios quizes favoritos y quizes pueden ser favoritos de varios usuarios
          ...
          if(count === 0) {   // la tabla se inicializa solo si está vacía
            Quiz.bulkCreate( 
              [ {pregunta: '¿Cuál es la capital de Italia?', respuesta: 'Roma', UserId: 2},
                {pregunta: '¿Cuál es la capital de Portugal?', respuesta: 'Lisboa', UserId: 2}]
              ).then(function(){
              Favourites.count().then(function(count) {
                if(count === 0) { //si la tabla está vacía se inicializa
                  Favourites.bulkCreate([ {UserId: 2, QuizId: 2}])
